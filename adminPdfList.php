...
<?php
	...
	$result = mysqli_query($con,"SELECT * FROM userreviewpage where userName = '$variableUser'");
	while ($row = mysqli_fetch_array($result)){
		$pdfLink =  $row['pdfLink'];
		if($pdfLink!=NULL){
			echo '<a class="link_button" href="adminReview.php?links=',base64_encode($pdfLink),'">' . $pdfLink . "</a>" . "<br>";
		}
	}
